...
describe('SlackChannel', () => {
  ...
  // --- File upload handling (F030 fix) ---

  describe('file upload handling', () => {
    it('allows file_share subtype through (was silently dropped before F030 fix)', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();
      ...
      const event = createMessageEvent({
        subtype: 'file_share',
        text: 'Here is the new build',
        user: 'U_USER_456',
        files: [{
          id: 'F123ABC',
          name: 'LaresCare.app.zip',
          size: 45000000,
          mimetype: 'application/zip',
          filetype: 'zip',
          url_private_download: 'https://files.slack.com/files-pri/T123/download/LaresCare.app.zip',
          permalink: 'https://larescare.slack.com/files/U123/F123ABC/LaresCare.app.zip',
        }],
      });
      await triggerMessageEvent(event);
      ...
      // Content should include file metadata with download URL
      const call = (opts.onMessage as any).mock.calls[0][1];
      expect(call.content).toContain('[File: LaresCare.app.zip]');
      expect(call.content).toContain('https://files.slack.com/files-pri/T123/download/LaresCare.app.zip');
      expect(call.content).toContain('45000000 bytes');
    });

    it('includes file attachments in message.files field', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();
      ...
      const event = createMessageEvent({
        ...
        files: [{
          ...
          mimetype: 'application/zip',
          filetype: 'zip',
          url_private_download: 'https://files.slack.com/download/test.app.zip',
        }],
      });
      await triggerMessageEvent(event);
      ...
      expect(call.files[0]).toEqual(expect.objectContaining({
        id: 'F456DEF',
        name: 'test.app.zip',
        size: 1000,
        mimetype: 'application/zip',
    ...
    it('handles file upload with no text (file-only message)', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();
      ...
      expect(opts.onMessage).toHaveBeenCalled();
      const call = (opts.onMessage as any).mock.calls[0][1];
      expect(call.content).toContain('[File: screenshot.png]');
    });

    it('handles file with no download URL (logs error, includes warning)', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();
      ...
      const call = (opts.onMessage as any).mock.calls[0][1];
      expect(call.content).toContain('UNAVAILABLE');
      expect(call.files[0].url_private_download).toBeUndefined();
    });

    it('still drops other subtypes like channel_join', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();
    ...
    it('handles multiple files in one message', async () => {
        ...
        files: [
          {
            ...
            mimetype: 'application/zip',
            filetype: 'zip',
            url_private_download: 'https://files.slack.com/download/build.app.zip',
          },
          {
            id: 'F2',
            ...
            mimetype: 'text/plain',
            filetype: 'txt',
            url_private_download: 'https://files.slack.com/download/release-notes.txt',
          },
        ],
      });
      await triggerMessageEvent(event);
      ...
      expect(call.content).toContain('build.app.zip');
      expect(call.content).toContain('release-notes.txt');
    });
  });
